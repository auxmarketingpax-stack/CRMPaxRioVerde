...
const apiClient = {
  async from(table){
    return {
      select: async ()=>fetch(`/api/${table}/list.php`).then(r=>r.json()).then(data=>({data, error:null})),
      insert: async (payload)=>fetch(`/api/${table}/create.php`,{method:'POST',body:JSON.stringify(payload)}).then(r=>r.json()),
      delete: async ()=>({}),
      update: async ()=>({})
    }
  }
...
(() => {
  ...
  function createClient() {
    ...
    apiClient = // removed supabase(cfg.supabaseUrl, cfg.supabaseAnonKey);
  }
  ...
  async function persistStagePositions(stages) {
    const updates = stages.map((stage, index) =>
      apiClient.from("stages").update({ position: index }).eq("id", stage.id)
    );
  ...
  async function ensureProfile() {
    ...
    const { data: profile, error } = await apiClient
      .from("profiles")
      .select("*")
  ...
  async function loadAppData() {
    const [stagesRes, leadsRes, profilesRes, historyRes] = await Promise.all([
      apiClient.from("stages").select("*").order("position", { ascending: true }),
      apiClient.from("leads").select("*").order("created_at", { ascending: false }),
      apiClient.from("profiles").select("*").order("full_name", { ascending: true }),
      apiClient.from("change_history").select("*").order("created_at", { ascending: false }).limit(300)
    ]);
  ...
  async function seedDefaultStages() {
    ...
    const { error } = await apiClient.from("stages").insert(defaults);
  ...
  async function logChange(action, entityType, entityId, description, payload = null) {
    ...
    const { error } = await apiClient.from("change_history").insert([row]);
  ...
  async function submitStage(event) {
    ...
    if (els.stageId.value) {
      ...
      const { error } = await apiClient.from("stages").update({ name: payload.name, color: payload.color, stage_type: payload.stage_type, custom_stage_type: payload.custom_stage_type, position: payload.position }).eq("id", els.stageId.value);
      if (error) return alert(`Erro no Supabase: ${error.message}`);
      ...
      const { data, error } = await apiClient.from("stages").insert([payload]).select().single();
      if (error) return alert(`Erro no Supabase: ${error.message}`);
  ...
  async function sendResetPasswordEmail() {
    ...
    const { error } = await apiClient.auth.resetPasswordForEmail(email, { redirectTo });
